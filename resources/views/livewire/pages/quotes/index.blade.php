...
new #[Layout('components.layout.app')] class extends Component
{
    ...
    public bool $showDeclineConfirm = false;

    public bool $showConvertConfirm = false;
    ...
    public function send(): void
    {
        $doc = Document::findOrFail($this->detailId);
        $this->authorize('update', $doc);
        app(DocumentService::class)->sendQuote($doc, Auth::user());
    }
    ...
    public function accept(): void
    {
        $doc = Document::findOrFail($this->detailId);
        $this->authorize('update', $doc);
        app(DocumentService::class)->acceptQuote($doc, Auth::user());
    }
    ...
    public function confirmDecline(): void
    {
        $doc = Document::findOrFail($this->detailId);
        $this->authorize('update', $doc);
        app(DocumentService::class)->declineQuote($doc, Auth::user());
    ...
    public function expire(): void
    {
        $doc = Document::findOrFail($this->detailId);
        $this->authorize('update', $doc);
        app(DocumentService::class)->expireQuote($doc, Auth::user());
    }
    ...
    public function confirmConvert(): void
    {
        $doc = Document::findOrFail($this->detailId);
        $this->authorize('update', $doc);
        $invoice = app(DocumentService::class)->convertQuoteToInvoice($doc, Auth::user());
        $this->showConvertConfirm = false;
        $this->closeDetail();
        session()->flash('success', "Quote converted to invoice {$invoice->document_number}.");
    }
...
<div class="flex items-center gap-1 px-6 pt-4 border-b border-line overflow-x-auto">
    @php
        $tabs = ['' => 'All', 'draft' => 'Draft', 'sent' => 'Sent', 'accepted' => 'Accepted', 'converted' => 'Converted', 'declined' => 'Declined', 'expired' => 'Expired'];
    @endphp
...
<div class="px-6 py-3 border-b border-line bg-surface-alt">
    <flux:input wire:model.live.debounce.300ms="search" placeholder="Search quotes…" size="sm" icon="magnifying-glass" class="max-w-xs" />
</div>
    ...
    <table class="w-full text-sm">
        ...
        <tbody>
            @forelse($rows as $quote)
                <tr wire:click="openDetail('{{ $quote->id }}')" class="border-t border-line hover:bg-surface-alt cursor-pointer">
                    <td class="px-4 py-3 font-mono text-xs text-ink">
                        {{ $quote->document_number ?? '—' }}
                        @if($quote->reference)
                            <span class="text-ink-muted block text-xs">{{ $quote->reference }}</span>
                        @endif
                    </td>
                    <td class="px-4 py-3 font-medium text-ink">{{ $quote->party?->business?->display_name ?? '—' }}</td>
                    <td class="px-4 py-3 text-ink-soft text-xs">{{ $quote->issue_date?->format('d M Y') ?? '—' }}</td>
                    <td class="px-4 py-3 text-right font-medium text-ink tabular-nums">
                        {{ $currencySymbol }}{{ number_format((float) $quote->total, 2) }}
                    </td>
                    <td class="px-4 py-3">
                        @php
                            $badgeClass = match($quote->status) {
                                ...
                                'declined', 'expired' => 'bg-red-50 text-danger',
...
{{-- ===== Create Modal ===== --}}
<flux:modal name="create-quote-modal" wire:model.self="showCreateModal" class="w-[480px]">
    <form wire:submit="createQuote" class="flex flex-col">
        <div class="p-6 border-b border-line">
            <flux:heading size="lg" class="font-semibold">New Quote</flux:heading>
        </div>
...
{{-- ===== Detail Flyout ===== --}}
<flux:modal name="quote-detail-flyout" flyout wire:model.self="showDetail" class="w-[760px]" @close="closeDetail">
    @if($detail)
            ...
            <div class="flex items-start justify-between">
                <div>
                    <div class="flex items-center gap-3">
                        <flux:heading size="lg" class="font-semibold">
                            {{ $detail->document_number ?? 'Draft Quote' }}
                        </flux:heading>
                        @php
                ...
                <div class="flex items-center gap-2">
                    ...
                    @if($detail->status === 'draft')
                        @can('update', $detail)
                            <flux:button wire:click="send" size="xs" variant="ghost" icon="paper-airplane">Send</flux:button>
                        @endcan
                    @endif

                    @if($detail->status === 'sent')
                        @can('update', $detail)
                            <flux:button wire:click="accept" size="xs" variant="ghost" icon="check">Mark Accepted</flux:button>
                            <flux:button wire:click="$set('showDeclineConfirm', true)" size="xs" variant="ghost" class="text-danger">Decline</flux:button>
                        @endcan
                    @endif

                    @if($detail->status === 'accepted')
                        @can('update', $detail)
                            <flux:button wire:click="$set('showConvertConfirm', true)" size="xs" variant="primary" icon="document-text">Convert to Invoice</flux:button>
                        @endcan
                    @endif

                    @if(in_array($detail->status, ['draft', 'sent']))
                        @can('update', $detail)
                            <flux:button wire:click="expire" size="xs" variant="ghost" class="text-ink-muted">Expire</flux:button>
                        @endcan
                    @endif
...
{{-- Decline Confirm --}}
<flux:modal name="decline-confirm" wire:model.self="showDeclineConfirm" class="w-[400px]">
    <div class="p-6">
        <flux:heading size="lg" class="font-semibold mb-2">Decline Quote?</flux:heading>
        <p class="text-sm text-ink-soft mb-6">Mark this quote as declined by the client.</p>
        <div class="flex gap-3 justify-end">
            <flux:button variant="ghost" wire:click="$set('showDeclineConfirm', false)">Cancel</flux:button>
            <flux:button variant="danger" wire:click="confirmDecline">Decline</flux:button>
        </div>
    </div>
</flux:modal>

{{-- Convert Confirm --}}
<flux:modal name="convert-confirm" wire:model.self="showConvertConfirm" class="w-[400px]">
    <div class="p-6">
        <flux:heading size="lg" class="font-semibold mb-2">Convert to Invoice?</flux:heading>
        <p class="text-sm text-ink-soft mb-6">A draft sales invoice will be created with the same lines. The quote will be marked as converted.</p>
        <div class="flex gap-3 justify-end">
            <flux:button variant="ghost" wire:click="$set('showConvertConfirm', false)">Cancel</flux:button>
            <flux:button variant="primary" wire:click="confirmConvert">Convert</flux:button>
        </div>
    </div>
</flux:modal>

{{-- Delete Confirm --}}
<flux:modal name="quote-delete-confirm" wire:model.self="showDeleteConfirm" class="w-[400px]">
    <div class="p-6">
        <flux:heading size="lg" class="font-semibold mb-2">Delete Quote?</flux:heading>
        <p class="text-sm text-ink-soft mb-6">This will permanently delete the draft quote and all its lines.</p>
        <div class="flex gap-3 justify-end">
