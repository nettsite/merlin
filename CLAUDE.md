...
- **VAT amounts are always stored ex-VAT, but the printed invoice amount is authoritative.** `DocumentLine.unit_price` and `line_total` are pre-tax; `tax_amount` is normally `calculateTotals()` as `line_total × tax_rate / 100`. The LLM prompt extracts amounts **as shown** on the invoice (not pre-stripped of VAT). `InvoiceProcessingService` detects VAT-inclusive extractions (when `sum(line_totals) ≈ extracted.total` and `taxTotal > 0`); for those lines it back-calculates the ex-VAT base **and derives VAT by subtraction (gross − net)** via the transient `DocumentLine::$taxAmountOverride`, so the stored gross matches the invoice to the cent (recomputing `rate × net` off the rounded ex-VAT base drifts a cent — e.g. 700.00 → 608.70 → 91.31 → 700.01).
